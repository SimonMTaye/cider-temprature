...
use "$data/generated/hi_hourly_temp_NOAA_indiatime.dta", clear 
	...
	collapse (mean) workday_temperature_c=temperature_c workday_heat_index=heat_index humidity, by(day month year)

	egen date = group(year month day)
	sort date
	
	* create lags
		foreach var of varlist workday_temperature_c workday_heat_index {
...
use "$data/generated/hi_analysis_daily.dta", clear
	...
	****** Merge lagged temperatue data with productivity data
  	...
  	preserve
			...
			egen mean_heat_index = mean(heat_index), by(day_in_study)
			egen mean_temperature_c = mean(temperature_c), by(day_in_study)
			keep mean_temperature_c mean_heat_index day_in_study
			duplicates drop
			...
			forvalues i = 1/10 {
				...
				gen l`i'_mean_heat_index = L`i'.mean_heat_index

				gen ld`i'_mean_temperature_c = F`i'.mean_temperature_c
				gen ld`i'_mean_heat_index = F`i'.mean_heat_index
			}
			tempfile mean_lags

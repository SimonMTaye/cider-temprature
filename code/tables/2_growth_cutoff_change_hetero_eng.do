...
use "$data/generated/hi_analysis_twoday.dta", clear
    ...
    local filename "$output/tables/table_growth_cutoff_changes_hetero_eng.tex"
    ...
    forvalues i=6/10 {
        forvalues english=0/1 {
            di "Run i: `i' and Computer: `computer'"
            qui reghdfe `dep_var' `indep_var' if (two_days > 2) & (two_days <= `i') & (english == `english'), `se_spec' 
